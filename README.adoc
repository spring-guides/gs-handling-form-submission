...
In Spring's approach to building websites, the controller handles HTTP requests.
These components are easily identified by the {Controller}[`@Controller`] annotation. The
`GreetingController` in the following listing handles GET
...
.Kotlin
----
include::complete-kotlin/src/main/kotlin/com/example/handlingformsubmission/GreetingController.kt[]
----
====
...
the view template. The `Greeting` object in the following code contains fields such as
...
.Kotlin
----
include::complete-kotlin/src/main/kotlin/com/example/handlingformsubmission/Greeting.kt[]
----
====
...
to render the form. The following listing shows the `greeting` template:
...
listing shows the result template:
...
listing shows the application class:
...
.Kotlin
----
include::complete-kotlin/src/main/kotlin/com/example/handlingformsubmission/HandlingFormSubmissionApplication.kt[]
----
====
...
Now that the website is running, visit http://localhost:8080/greeting, where you see the
following form:
